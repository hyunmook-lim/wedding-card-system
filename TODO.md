...
- [x] priority 미디어 완료 후 청첩장 화면 전환
  - intro의 priority 자원은 계속 초기 진입 조건으로 유지한다.
  - regular 미디어는 화면 전환 뒤 백그라운드에서 계속 로드한다.
  - VideoGreeting2는 첫 프레임이 준비되기 전 로딩 UI를 표시하고, 전체 프레임이 준비될 때까지 첫 프레임을 유지한다.
...
- [x] 현재 섹션과 다음 섹션만 미리 로딩하는 스케줄러 구현

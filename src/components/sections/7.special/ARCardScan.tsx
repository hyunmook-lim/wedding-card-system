...
export default function ARCardScan({ config, onClose }: SectionProps) {
  ...
  const handleContainerClick = () => {
    if (status === 'idle') {
      startAR();
    } else if (videoRef.current && isDetected) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-10 pointer-events-none flex flex-col items-center justify-center">
        <div className="w-full h-full relative pointer-events-none flex flex-col items-center justify-center">
          ...
          <AnimatePresence>
            {status === 'idle' && (
              <motion.div 
                ...
                className="relative z-10 flex flex-col items-center px-8 w-full max-w-[320px] pointer-events-auto"
              >
                ...
                <div className="mb-12 text-center">
                  <Typography className="text-[0.8rem] text-black/50 leading-relaxed font-light whitespace-pre-line">
                    {subtitle}
                  </Typography>
                </div>
                <button
                  onClick={(e) => { e.stopPropagation(); startAR(); }}
                  className="relative w-full py-4 rounded-full text-[0.65rem] tracking-[0.3em] font-bold uppercase overflow-hidden"
                  style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.1)', color: 'rgba(0,0,0,0.6)' }}
                >
                  카메라 시작하기
                </button>
              </motion.div>
          ...
          <AnimatePresence>
            {status === 'loading' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-[#fffdf7]/80 backdrop-blur-sm">
          ...
          <AnimatePresence>
            {status === 'active' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-10 w-full h-full pointer-events-none">
                <div className={`absolute inset-0 flex flex-col items-center justify-center pointer-events-none transition-opacity duration-500 ${isDetected ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
                ...
                {isDetected && (
                  <button 
                    ...
                    className="absolute bottom-10 right-8 z-20 w-12 h-12 rounded-full flex items-center justify-center pointer-events-auto bg-white/5 backdrop-blur-md border border-white/10"
                  >
          ...
          {/* ── Error State ── */}
          {status === 'error' && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/95 z-30 px-10 text-center text-white pointer-events-auto">
              <div className="w-16 h-16 rounded-full flex items-center justify-center bg-red-500/10 border border-red-500/20 mb-6">📷</div>
              <p className="text-red-400/80 text-sm font-light mb-8 whitespace-pre-line">{errorMsg}</p>
              <button onClick={(e) => { e.stopPropagation(); startAR(); }} className="px-8 py-3 rounded-full border border-amber-400/20 text-amber-400 text-xs uppercase">다시 시도</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-[100] pointer-events-none">
        <button
          onClick={(e) => {
            e.stopPropagation();
            onClose?.();
          }}
          className="w-10 h-10 rounded-full flex items-center justify-center transition-transform active:scale-90 pointer-events-auto shadow-xl"
          style={{
            background: status === 'idle' ? 'rgba(0,0,0,0.03)' : 'rgba(0,0,0,0.4)',
            backdropFilter: 'blur(12px)',
            border: status === 'idle' ? '1px solid rgba(0,0,0,0.05)' : '1px solid rgba(255,255,255,0.15)',
          }}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={status === 'idle' ? 'black' : 'white'} strokeWidth="2.5" strokeLinecap="round">
            <polyline points="15 18 9 12 15 6" />
          </svg>
        </button>
        <Typography className={`text-[9px] tracking-[0.4em] font-bold uppercase transition-opacity duration-500 ${status === 'idle' ? 'text-black/30' : 'text-white/50'}`}>
          AR Gallery
        </Typography>
      </div>

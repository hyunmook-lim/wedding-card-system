...
function DiagonalPhoto({
  ...
  // React 상태 대신 직접 DOM src 조작으로 현재 카드와 다음 카드만 메모리에 유지한다.
  const imgRef = useRef<HTMLImageElement>(null);
  const isLoadedRef = useRef(false);

  const updateImageSource = useCallback((progressOffset: number) => {
    if (!imgRef.current) return;

    const shouldLoad = progressOffset > -PRELOAD_BEHIND_COUNT && progressOffset < PRELOAD_AHEAD_COUNT;
  ...
  // 마운트 시 현재 보이는 카드와 다음 카드만 요청한다.
  useEffect(() => {
    updateImageSource(offset.get());
  }, [offset, updateImageSource]);

  useMotionValueEvent(offset, "change", (latest) => {
    updateImageSource(latest);
  });
      ...
      >
        ...
        >
          <motion.img 
            ...
            className="relative block h-[75vw] w-[75vw] max-h-[75vw] max-w-[75vw] object-contain text-transparent sm:h-[300px] sm:w-[300px] sm:max-h-[300px] sm:max-w-[300px]"
            loading="lazy"
            decoding="async"
            onError={(event) => {

...
import { motion, useMotionValueEvent, useScroll, AnimatePresence, Variants } from 'framer-motion';
...
// --- (2) GlassmorphismCalendar Main Component ---
export default function GlassmorphismCalendar({ config, isVisible }: SectionProps) {
                ...
                {weekDays.map((day: string, idx: number) => (
                  <div key={day} className="text-center">
                    <Typography className={cn(
                      ...
                      idx === 0 ? "text-red-300/60" : idx === 6 ? "text-blue-300/60" : "text-black/20"
                    )}>
                      {day}
                    </Typography>
                  </div>

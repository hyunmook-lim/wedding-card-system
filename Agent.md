...
- gallery 설정 이미지의 선행 로딩 대상은 처음 4장이다.
...
AlbumGallery는 33장 전체에 `src`를 주지 않는다. 현재 카드 기준 앞 2장과 다음 3장만 DOM 이미지에 연결하며, 범위를 벗어난 카드에서는 `src` 속성을 제거한다. 카드 영역은 고정된 배경으로 유지하고, 전체 화면 `ImageViewer`는 사용자가 열었을 때 해당 원본 이미지만 요청한다.
